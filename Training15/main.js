...
let contractAddress = "0xD93b3043ECA9FA1360B01B43a9ca172e817dE919";
let contractAbi = [
    {
      "inputs": [
        {
...
const addVote = async() => {
    if(WALLET_CONNECTED != 0) {
        let name = document.getElementById("vote");
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        await provider.send("eth_requestAccounts", []);
        const signer = provider.getSigner();
...
const getAllCandidates = async() => {
    if(WALLET_CONNECTED != 0) {
        ...
        let candidates = await contractInstance.getAllVotesOfCandiates();
